...
- To add or update a semester, scrape only that semester and import it with `py backend/scripts/import_alma_json_to_d1.py --input <single-period json> --incremental --apply`: it replaces only the input's periods, keeps existing ids, and fails if any other period or user data changed (dry run without `--apply`). A full rebuild re-seeds the existing D1 **in place** — do not create/swap a DB: `py backend/scripts/import_alma_json_to_d1.py --input <courses_multi_semester.json> --apply --skip-create --skip-swap --skip-migrate`. The full seed DELETEs all catalog rows and reinserts only the periods present in the JSON, so keep every period you want to retain in the input. See that script's docstring for the D1 remote-import limits (compound-SELECT coalescing, Durable-Object reset) it works around.
