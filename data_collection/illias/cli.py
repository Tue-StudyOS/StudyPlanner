...
def build_parser() -> argparse.ArgumentParser:
    ...
    export.add_argument("--alma-db", type=Path, default=DEFAULT_ALMA_DB)
...
def _insert_statement_with_expressions(table: str, columns: list[str], values: list[str]) -> str:
    column_list = ", ".join(f'"{column}"' for column in columns)
    value_list = ", ".join(values)
    return f'INSERT INTO "{table}" ({column_list}) VALUES ({value_list});'


def _load_alma_match_targets(alma_db: Path, course_ids: list[int]) -> dict[int, dict[str, object]]:
    if not course_ids or not alma_db.is_file():
        return {}
    with connect(alma_db) as connection:
        placeholders = ", ".join("?" for _ in course_ids)
        rows = connection.execute(
            f"""
            SELECT id, number, unit_id, period_id, title
            FROM courses
...
def _d1_course_id_expression(match_row: object, targets_by_id: dict[int, dict[str, object]]) -> str:
    row = dict(match_row)
    matched_id = row.get("alma_course_id")
    if matched_id is None:
        return "NULL"
    target = targets_by_id.get(int(matched_id))
    if not target:
        return "NULL"
    ...
    if period_id and number:
        return (
            "(SELECT id FROM courses "
            f"WHERE period_id = {_sql_literal(period_id)} AND number = {_sql_literal(number)} "
            "ORDER BY id LIMIT 1)"
        )
    if period_id and unit_id:
        return (
            "(SELECT id FROM courses "
            f"WHERE period_id = {_sql_literal(period_id)} AND unit_id = {_sql_literal(unit_id)} "
            "ORDER BY id LIMIT 1)"
        )
    if period_id and title:
        return (
            "(SELECT id FROM courses "
            f"WHERE period_id = {_sql_literal(period_id)} AND title = {_sql_literal(title)} "
            "ORDER BY id LIMIT 1)"
        )
    return "NULL"
...
def _run_export_sql(args: argparse.Namespace) -> None:
    ...
    matched_ids = [
        int(row["alma_course_id"])
        for row in matches
        if row["alma_course_id"] is not None
    ]
    alma_targets = _load_alma_match_targets(args.alma_db, matched_ids)
    ...
    with args.out.open("w", encoding="utf-8") as handle:
        ...
        for row in matches:
            ...
            values = [
                _d1_course_id_expression(row, alma_targets)
                if column == "alma_course_id"
                else _sql_literal(row[column])
                for column in columns
            ]
            handle.write(_insert_statement_with_expressions("illias_alma_matches", columns, values) + "\n")
